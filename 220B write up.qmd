...
The ranges are determined to best partition the data ased on the histogram shown: 
...
The first topic of interest was to study if there is an association between infection and vaccination status.The is done by a contingency table chi square test. The null hypothesis is the variables are independent and the alternative is that they are dependent. To investigate the potential impact demographic variables have on this association, multiple tests were performed.To sum up the results: the association between infection status and vaccination status is statistically significant.The association depends on gender. However,the association does not seem to depend on race and age.
...
First examine the confusion matrices, the model with the lab tests performed better since it has a better true positive and true negative rate. The ROC curves also showed that the model with lab tests performed better.(The closer the ROC curve is to the upper corner, the higher the accuracy is). The model with the limited variables does not perform well to predict true negatives, and the false positive rate is high.The lack of lab tests does affect how accurate the model is. The model with the lab tests performs relatively well, the true positive and the true negative rates are relatively high.

Overall, the model performed with the lab tests performed better when it comes to prediction at this specific threshold. 


**Recommendations**

This model will be work in process and keep improving with more knowledge.

Here are topics that can be looked into more. 

1.Under dispersion 

when fitting the model for part two, there is a potential display of under dispersion of the model. It is possible that a quasi binomial model can fit the data. 


2.Threshold chosen arbitrarily

The threshold to convert predicted probabilities to positive or negative cases in chosen to strike a balance between specificity and sensitivity. However, that could not be the best strategy, as a different threshold can greatly affect what the predictions are.
